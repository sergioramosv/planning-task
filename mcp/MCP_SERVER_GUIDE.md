...
Or just edit `mcp/.env` with the same values.
...
To use with Claude Code or other Claude clients, configure it in your settings:
...
echo '{"method":"tools/list","params":{"projectId":"test-project"}}' | node dist/index.js
```
...
Use actual environment variables instead of .env files:
...
1. ✅ Never commit `serviceAccountKey.json` or `.env.local`
2. ✅ Use environment variables for production
3. ✅ Restrict Firebase security rules to authorized users only
4. ✅ Rotate service account keys periodically
5. ✅ Monitor Firebase console for suspicious activity

### Running as a Service

On Linux/macOS, you can use `systemd` or `supervisor`:

```ini
[program:planning-task-mcp]
command=/usr/bin/node /path/to/mcp/dist/index.js
autostart=true
autorestart=true
environment=FIREBASE_SERVICE_ACCOUNT_PATH=/path/to/key.json,FIREBASE_DATABASE_URL=https://...
```
...
- 📖 Read [SETUP.md](./mcp/SETUP.md) for setup issues
- 📖 Read [README.md](./mcp/README.md) for technical details
