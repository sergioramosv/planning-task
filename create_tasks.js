...
/**
 * Script para crear todas las tareas del roadmap
 * Uso: node create_tasks.js
 */
...
// Function to send requests to MCP server via stdin/stdout
async function createTaskViaMCP(task) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      reject(new Error(`Timeout creating task: ${task.title}`));
    }, 10000);
...
async function main() {
  ...
  for (let i = 0; i < TASKS.length; i++) {
    const task = TASKS[i];
    try {
      console.log(`[${i + 1}/${TASKS.length}] Creando: ${task.title}...`);
      await createTaskViaMCP(task);
      successCount++;
